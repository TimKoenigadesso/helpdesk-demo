...
export function TicketForm({ onCreated }: Props) {
  ...
  const handleReporterNameChange = (value: string) => {
    setReporterName(value);
    if (value.length > 100) {
      setReporterNameError('Der Name darf maximal 100 Zeichen lang sein.');
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (reporterName.length > 100) return;
    setLoading(true);
    try {
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="px-6 pb-6">
        ...
        <div className="mb-3">
          ...
          >
            Ihr Name <span className="text-gray-400 font-normal">(max. 100 Zeichen)</span>
          </label>
          <input
            id="ticket-reporter-name"
            type="text"
            placeholder="Vor- und Nachname des Melders"
            value={reporterName}
          ...
          />
          {reporterNameError && (
            <p className="mt-1 text-xs text-red-600" data-testid="reporter-name-error">
              {reporterNameError}
            </p>
          )}
          {reporterName.length > 0 && !reporterNameError && (
            <p className="mt-1 text-xs text-gray-400" data-testid="reporter-name-counter">
              {reporterName.length}/100 Zeichen
            </p>
          )}
        </div>
