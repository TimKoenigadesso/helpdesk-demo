...
def create_ticket(ticket: TicketCreate):
    ...
    reporter_name = ticket.reporter_name.strip()[:100] if ticket.reporter_name and ticket.reporter_name.strip() else None
    with get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO tickets (title, description, priority, reporter_name) VALUES (?, ?, ?, ?) RETURNING *",
            (ticket.title, ticket.description, priority, reporter_name),
        )
        row = cur.fetchone()
    return dict(row)
